export interface FiremoteCardConfig {
  device_name: string;
...
class FiremoteCard extends HTMLElement {
  private _config: FiremoteCardConfig | null = null;
  private _hass: any;
  ...
  public setConfig(config: Partial<FiremoteCardConfig>): void {
    if (!config?.device_name && !config?.media_player_entity) {
  ...
  private render(): void {
    ...
    const volumeLevel = typeof mediaState?.attributes?.volume_level === 'number' ? mediaState.attributes.volume_level : null;
  ...
  private attachListeners(): void {
    const buttons = Array.from(this.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[data-action]'));
    buttons.forEach(button => {
      button.addEventListener('click', () => this.handleButton(button.dataset.action || ''));
    });

    const sendTextButton = this.shadowRoot!.querySelector<HTMLButtonElement>('#firemote-text-send');
    const textInput = this.shadowRoot!.querySelector<HTMLInputElement>('#firemote-text-input');
    if (sendTextButton) {
      sendTextButton.addEventListener('click', () => this.handleSendText(textInput?.value || ''));
    }

    const volumeSlider = this.shadowRoot!.querySelector<HTMLInputElement>('#firemote-volume-slider');
    if (volumeSlider) {
      volumeSlider.addEventListener('change', () => this.handleVolumeSlider(Number(volumeSlider.value) / 100));
    }
  }

  private handleButton(action: string): void {
    switch (action) {
      case 'power':
  ...
  private handleSendText(value: string): void {
  ...
  private handleVolumeSlider(level: number): void {
  ...
  private getVolumeLevel(): number | null {
    if (!this._config?.media_player_entity || !this._hass) {
      return null;
    }
  ...
  private callService(service: string, data: Record<string, unknown>): void {
    ...
    const serviceData: Record<string, unknown> = {
      device_name: this._config.device_name || this._config.media_player_entity,
      ...data
    };
